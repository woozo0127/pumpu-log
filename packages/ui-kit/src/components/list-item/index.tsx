...
const ListItem = React.forwardRef<React.ComponentRef<typeof View>, ListItemProps>(
  ({ icon, title, subtitle, rightElement, className, ...props }, ref) => {
    return (
      ...
      >
        {icon != null && (
          <View className="w-11 h-11 rounded-md bg-card items-center justify-center">{icon}</View>
        )}
        ...
        {rightElement !== undefined ? (
          ...
          <ChevronRight size={20} color={colors['foreground-tertiary']} />
        )}
      </Pressable>
    );
  },
